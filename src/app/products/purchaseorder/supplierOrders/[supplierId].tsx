...
const EnhancedOrderRow = withObservables(['order'], ({ order }: { order: PurchaseOrder }) => ({
  order: order.observe(),
}))(function OrderRow({ order, onPress }: { order: PurchaseOrder; onPress: () => void }) {
...
function BulkPayModal({ visible, totalDue, onClose, onPay }: {
  ...
  const handlePay = async () => {
    ...
    if (val > totalDue) { Alert.alert(`Amount exceeds total payable ${totalDue.toFixed(2)}`); return }
    setLoading(true)
    await onPay(val)
    setLoading(false)
    setAmount('')
          ...
          <View style={s.modalSheet}>
            ...
            <View style={s.modalActions}>
              ...
              <Pressable onPress={handlePay} style={s.confirmBtn} disabled={loading}>
                {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={s.confirmBtnText}>Record Payment</Text>}
              </Pressable>
            </View>
          </View>
        </View>
