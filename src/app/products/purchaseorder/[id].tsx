        ...
        <View style={s.modalOverlay}>
          <View style={s.modalSheet}>
            ...
            <TextInput
              ...
              inputMode={'numeric'}
              ref={(ref) => { if (visible) ref?.focus() }}
            />

            <Pressable onPress={() => setAmount(due.toFixed(2))} style={s.payFullBtn}>
...
// ─── Detail ───────────────────────────────────────────────────────────────────
function PurchaseOrderDetail({order, supplier, items, transactions}: {
  order: PurchaseOrder
  supplier: any
  ...
  return (
    <View style={{flex: 1, backgroundColor: '#f9fafb'}}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Purchase Order</Text>
        <View style={{width: 30}}/>
      </View>
...
// ─── withObservables ──────────────────────────────────────────────────────────
const EnhancedDetail = withObservables(['order'], ({order}: {order: PurchaseOrder}) => ({
  order: order.observe(),
  supplier: order.supplier.observe(),
...
export default function PurchaseOrderDetailScreen() {
  ...
  const Screen = withObservables(['id'], ({id}: {id: string}) => ({
    order: database.get<PurchaseOrder>('purchase_orders').findAndObserve(id),
  }))(({order}: {order: PurchaseOrder}) => <EnhancedDetail order={order}/>)
  return <Screen id={id}/>
}
...
const s = StyleSheet.create({
  ...
  cardTitle: {fontSize: 13, fontFamily: 'InterMedium', color: '#9ca3af', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5},
  cardRow: {flexDirection: 'row', alignItems: 'center', gap: 12},
  supplierIcon: {width: 44, height: 44, borderRadius: 12, backgroundColor: '#f3f4f6', alignItems: 'center', justifyContent: 'center'},
  supplierIconText: {fontSize: 18, fontFamily: 'InterBold', color: '#374151'},
  ...
  txRow: {flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f5f5f5'},
