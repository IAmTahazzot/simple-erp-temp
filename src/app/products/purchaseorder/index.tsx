...
const PurchaseOrderRow = withObservables(['order'], ({order}: {order: PurchaseOrder}) => ({
  order: order.observe(),
  supplier: order.supplier.observe(),
}))(({order, supplier, onPress}: any) => {
...
function PurchaseOrders({orders}: {orders: PurchaseOrder[]}) {
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={s.empty}>No purchase orders found</Text>}
      />
    </View>
  )
}

function PurchaseOrdersLoader({orders}: {orders?: PurchaseOrder[]}) {
